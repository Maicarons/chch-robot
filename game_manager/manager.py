...
from .recorder import save_game_record
...
class GameManager:
    ...
    def _validate_player_move(self, old_fen: str, new_fen: str) -> Optional[str]:
        ...
        try:
            # 比较新旧 FEN，提取可能的走法
            move = FENUtils.extract_move_from_fen_change(old_fen, new_fen)
    ...
    def detect_player_move(self) -> Optional[str]:
        ...
        while attempts < max_attempts:
            ...
            if current_fen and current_fen != previous_fen:
                ...
                # 提取走法（比较两个 FEN 的差异）
                move = FENUtils.extract_move_from_fen_change(previous_fen, current_fen)
    ...
    def _extract_move_from_fen_change(self, old_fen: str, new_fen: str) -> Optional[str]:
        """已迁移至 ``xq_utils.FENUtils.extract_move_from_fen_change``，保留向后兼容包装。"""
        return FENUtils.extract_move_from_fen_change(old_fen, new_fen)
